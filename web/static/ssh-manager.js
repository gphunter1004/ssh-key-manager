...
function updateUI() {
    ...
    if (isAuthenticated) {
        ...
        // 네비게이션 업데이트 (권한에 따른 메뉴 표시/숨김)
        if (typeof ViewManager !== 'undefined' && ViewManager.updateNavigation) {
            ViewManager.updateNavigation();
        }
        
        // 현재 뷰 표시
        if (typeof ViewManager !== 'undefined' && ViewManager.showView) {
            ViewManager.showView(AppState.currentView || 'keys');
        } else {
            console.warn('⚠️ ViewManager를 사용할 수 없음');
        }
